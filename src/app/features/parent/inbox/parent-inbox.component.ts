...
import { ParentInboxApiService } from '../../../core/api/parent-inbox-api.service';
import { StudentInboxItem, AttendanceDayDto } from '../../../core/api/student-inbox-api.service';
...
type Tab = 'alerts' | 'schoolNotices' | 'messages' | 'homework' | 'attendance';
...
@Component({
  ...
  styleUrl: './parent-inbox.component.css'
})
export class ParentInboxComponent implements OnInit {
  private api = inject(ParentInboxApiService);
  private ws  = inject(InboxWebSocketService);

  activeTab  = signal<Tab>('alerts');
  loading    = signal(true);
  ...
  alertBadge  = computed(() => this.alerts().filter(i => i.readStatus === 'UNREAD').length);
  noticeBadge = computed(() => this.schoolNotices().filter(i => i.readStatus === 'UNREAD').length);
  msgBadge    = computed(() =>
